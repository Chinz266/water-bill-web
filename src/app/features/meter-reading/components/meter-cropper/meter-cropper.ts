  ...
  readonly autoBoxNote = computed<string | null>(() => {
    ...
    if (!box) {
      return this.usedFallback()
        ? 'ระบบค้นหาแถวตัวเลขไม่พบ — วางกรอบไว้เหนือกึ่งกลางภาพให้ก่อน (ตำแหน่งมาตรฐานของแถบตัวเลข) กรุณาปรับกรอบให้ตรงก่อนยืนยันครับ'
        : null;
    }
    ...
    return box.source === 'border'
      ? 'ระบบวางกรอบให้เบื้องต้นแล้ว — กรอบนี้ยังรวมเลขทศนิยมสีแดงอยู่ กรุณาย่อกรอบให้เหลือเฉพาะเลขสีดำก่อนยืนยันครับ'
      : 'ระบบวางกรอบให้แล้ว — กรุณาตรวจสอบว่าครอบเลขสีดำครบทุกหลัก ปรับกรอบได้ตามต้องการครับ';
  });
