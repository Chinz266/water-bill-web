...
import { GPS_POLICY } from '../../../core/measurement.constants';
...
/**
 * ระยะต่ำสุดที่ทิศระหว่างสองจุด "เป็นข้อมูล" ไม่ใช่เสียงรบกวน (เมตร)
 *
 * เคยเป็น 3 ม. ตาม GPS_FLOOR_M แต่เหตุผลที่มิเตอร์ห่างกัน 30 ซม. แยกไม่ออก ใช้ได้กับ
 * ทุกระยะที่สั้นกว่าความคลาดของเครื่อง (3-10 ม. ที่โล่ง, 10-30 ม. ใต้ชายคา) ไม่ได้หยุดที่ 30 ซม.
 *
 * ในเขตนี้ตัวที่ตอบว่าตัวไหนของบ้านไหนได้จริงคือ sequence_index ที่คนจดไว้ล่วงหน้า
 *
 * ═══ ขยับจาก 10 ม. มาเป็น 15 ม. ตามผลการทดลอง ═══
 * การทดลองภาคสนาม (meter-bill.xlsx ตารางที่ 4.9) วัดมิเตอร์ 10 คู่ที่ติดกัน พบว่ามีเพียง
 * 2 คู่ (P06 และ P10) ที่หมุดห่างกันเกิน 15 ม. จนพ้นเขตสัญญาณรบกวน — และเฉพาะคู่ที่ทิศจริง
 * เป็นซ้าย/ขวา ระบบตอบถูก 1 จาก 4 คู่ เท่ากับเดาสุ่มพอดี เลข 10 เดิมเป็นค่าที่เลือกเอง
 * ไม่ใช่ค่าที่ข้อมูลชี้ ทั้งการจำลอง (ชี้ 20 ม.) และภาคสนาม (ชี้ 15 ม.) บอกตรงกันว่าต่ำเกิน
 *
 * ผลของการขยับคือป้ายทิศ**ขึ้นน้อยลง** ไม่ใช่มากขึ้น — เคสที่หายไปตกไปใช้ sequence_index
 * ซึ่งเชื่อได้จริงอยู่แล้ว จึงเป็นทิศที่ผิดพลาดได้อย่างปลอดภัยกว่า
 */
export const SIDE_FLOOR_M: number = GPS_POLICY.directionTrustM;
...
/**
 * เพดานของเกณฑ์เชื่อทิศ (เมตร) — ตรงกับ RelativeDirectionUtil.DIRECTION_TRUST_M
 *
 * ก่อนมีเพดานนี้ บ้านที่ยังไม่มีประวัติได้เกณฑ์ √(20²+20²) ≈ 28.3 ม. ซึ่งกว้างกว่าระยะ
 * ห่างจริงระหว่างบ้านส่วนใหญ่ ป้ายจึงไม่เคยขึ้นเลยสำหรับหมู่บ้านใหม่
 *
 * ⚠️ ต้องแก้พร้อม SIDE_FLOOR_M เสมอ — ตั้งเพดานต่ำกว่าพื้นแล้วพื้นถูกกลืนทิ้งทั้งค่า
 *    (อ้าง GPS_POLICY.directionTrustM ตัวเดียวกันทั้งคู่ ทั้งสองค่าจึงขยับพร้อมกันเสมอ)
 * ⚠️ 15 ม. เป็นค่าที่ภาคสนามชี้ ส่วนการจำลองชี้ 20 ม. — ป้ายในช่วง 15-20 ม. ยังผิดได้
 */
export const SIDE_TRUST_M: number = GPS_POLICY.directionTrustM;
...
export function sideFloorFor(spreadA: number | null, spreadB: number | null): number {
  ...
  // ⚠️ พื้นกับเพดานเท่ากันที่ 15 ม. ฟังก์ชันจึงคืน 15 เสมอ — การปรับตามความแม่นรายคู่
  //    ถูกปิดผลโดยตั้งใจ สูตรยังอยู่ครบ แยกสองค่าออกจากกันเมื่อไหร่ก็กลับมาทำงานทันที
  return Math.min(Math.max(SIDE_FLOOR_M, Math.hypot(a, b)), SIDE_TRUST_M);
}
